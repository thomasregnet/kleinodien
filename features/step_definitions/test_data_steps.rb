...
def release_ref # Sepultura - Arise
  @release_ref ||= BrainzReleaseReference.from_code(
    '7452f8c9-f9bc-3ca7-859e-3220e57e4e4a'
  )
end

def release_group_ref
...
When(/^I define test data$/) do
  TestData.define(:brainz_arise) do |testset|
    ...
    testset.define do |subset| 
      subset.add(:brainz_release_group, release_group_ref.code)
    end
  end
end
...
Then(/^I can retrieve that data$/) do
  ...
  subset = TestData.retrieve(:brainz_arise, 1)
  expect(subset.fetch(release_ref)).to match(/Arise/)
  expect(subset.fetch(artist_ref)).to match(/Biafra, Jello/)
  expect(subset.fetch(release_group_ref))
    .to match(/The Sky Is Falling and I Want My Mommy/)
end
