...
RSpec.describe Import::Cache, type: :model do
  before(:each) do
    ...
    @faked_reference = FakeReference.new(code: 'abc')
  end
  ...
  specify 'write and read known' do
    @cache.store_discogs(@faked_reference, 'foo')
    expect(@cache.fetch_discogs(@faked_reference)).to eq 'foo'
  end

  specify 'write and read requirements' do
    cache = Import::Cache.new
    cache.require_discogs(@faked_reference)
    expect(cache.required['discogs'][0]).to eq(@faked_reference.to_key)
  end
  ...
  it 'stores only one unique uri for a requirement' do
    ...
    3.times do
      ...
      @cache.require_tmdb(@faked_reference)
    end
  ...
  it '#fetch_<source_name>! raises Import::CacheMissingEntry on missing data' do
    expect { @cache.fetch_brainz!(@faked_reference) }
