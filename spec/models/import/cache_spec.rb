...
RSpec.describe Import::Cache, type: :model do
  before(:each) do
    ...
    @fakeed_foreign_id = FakeForeignId.new(value: 'abc')
  end
  ...
  specify 'write and read known' do
    @cache.store_discogs(@fakeed_foreign_id, 'foo')
    expect(@cache.fetch_discogs(@fakeed_foreign_id)).to eq 'foo'
  end

  specify 'write and read requirements' do
    cache = Import::Cache.new
    cache.require_discogs(@fakeed_foreign_id)
    expect(cache.required['discogs'][0]).to eq(@fakeed_foreign_id.cache_key)
  end
  ...
  it 'stores only one unique uri for a requirement' do
    ...
    3.times do
      ...
      @cache.require_tmdb(@fakeed_foreign_id)
    end
  ...
  it '#fetch_<source_name>! raises Import::CacheMissingEntry on missing data' do
    expect { @cache.fetch_brainz!(@fakeed_foreign_id) }
