...
RSpec.describe Station, type: :model do
  ...
  it "must have a case insensitive unique name" do
    clone = Station.new(name: @station.name.upcase)
    expect { clone.save! }.to raise_error
  end

  it "is unique with a disambiguation" do
    clone = Station.new(name: @station.name, disambiguation: 'other one')
  ...
  it "fails if a name-disambiguation pair already exists" do
    ...
    clone = Station.new(
      name: @station.name, disambiguation: @station.disambiguation)
    expect { clone.save! }.to raise_error
  end
end
