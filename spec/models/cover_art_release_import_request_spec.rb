...
RSpec.describe CoverArtReleaseImportRequest, type: :model do
  include_examples 'for ImportRequests', :cover_art_release_import_request
  include_examples(
    'for code fields that must be an uuid',
    :cover_art_release_import_request
  )
  ...
  describe '#to_uri' do
    ...
    let(:import_request) { FactoryBot.build(:cover_art_release_import_request, code: code) }
